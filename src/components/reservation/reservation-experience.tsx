...
function ContactStep({ form }: { form: UseFormReturn<ReservationInput> }) {
  return <><StepHeading eyebrow="Stay in touch" title="Who is the reservation for?" description="We will use these details for reservation updates only." /><div className="grid gap-5 sm:grid-cols-2"><Field label="Full name" error={form.formState.errors.name?.message} className="sm:col-span-2"><Input autoComplete="name" placeholder="John Smith" {...form.register("name")} /></Field><Field label="Email" error={form.formState.errors.email?.message}><Input type="email" autoComplete="email" placeholder="john@example.com" {...form.register("email")} /></Field><Field label="Phone" error={form.formState.errors.phone?.message}><Input type="tel" autoComplete="tel" placeholder="+62 812 3456 7890" {...form.register("phone")} /></Field></div><Input tabIndex={-1} autoComplete="off" className="hidden" aria-hidden="true" {...form.register("website")} /></>;
}
...
function Field({ label, error, children, className }: { label: string; error?: string; children: React.ReactNode; className?: string }) { return <div className={className}><Label>{label}</Label><div className="mt-2">{children}</div>{error ? <p className="mt-1.5 text-sm text-destructive">{error}</p> : null}</div>; }
